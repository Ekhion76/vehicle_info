            ...
            for _, vehicle in pairs(GetGamePool('CVehicle')) do
                if #(GetEntityCoords(vehicle) - pos) <= 80 then
                    ...
                    tempClosestVehicles[vehicle] = info
                            ...
                            --.. pattern:format('doorStatus', GetVehicleDoorLockStatus(vehicle))
                            --.. pattern:format('entityType', GetEntityType(vehicle))
                            --.. pattern:format('entityMission', IsEntityAMissionEntity(vehicle))
                            --.. pattern:format('onAllWheels', IsVehicleOnAllWheels(vehicle))
                            --.. pattern:format('gravity', GetVehicleGravityAmount(vehicle))
                            --.. pattern:format('entityScript', GetEntityScript(vehicle) or '-')
                            --.. pattern:format('attachedToTrailer', IsVehicleattachedToTrailer(vehicle))
                            --.. pattern:format('getVehicleTrailerVehicle', GetVehicleTrailerVehicle(vehicle))
                            --.. pattern:format('getEntityattachedTo', GetEntityattachedTo(vehicle))
                            --.. pattern:format('health', round(GetEntityHealth(vehicle)), 2)
                            --.. pattern:format('isPointOnRoad', IsPointOnRoad(vehicleCoords))
                            --.. pattern:format('velocity', GetEntityRotationVelocity(vehicle))
                            --.. pattern:format('velocity', GetEntityVelocity(vehicle))
